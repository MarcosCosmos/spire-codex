...
vi.mock("@/app/[locale]/cards/[id]/CardDetail", () => ({
  default: function CardDetail() {
    return null;
  },
}));
vi.mock("@/app/[locale]/events/[id]/EventDetail", () => ({
...
function healthy(url: string): Response {
  ...
  if (url.includes("/api/events/"))
    return ok({ id: "TRIAL", name: "Le procès", description: "Un choix." });
  if (url.includes("/api/runs/stats/"))
    return ok({ picks: 10, win_rate: 0.5, total_runs: 100 });
  if (url.includes("/api/runs/community-stats"))
    return ok({
      events: [
        {
          id: "TRIAL",
...
describe("localized entity pages", () => {
  it("renders the community stats on a localized card page and emits no FAQ schema", async () => {
    ...
    const detail = tree.find((el) => el.type === CardDetail) as
      ReactElement<{ initialStats: unknown }> | undefined;
    expect(detail?.props.initialStats).toMatchObject({ picks: 10 });
    const jsonLd = tree.find((el) => el.type === JsonLd) as
      ReactElement<{ data: unknown }> | undefined;
  ...
  it("renders the vote distribution on a localized event page and emits no FAQ schema", async () => {
    ...
    const detail = tree.find((el) => el.type === EventDetail) as
      ReactElement<{ voteStats: unknown }> | undefined;
