...
const ARBITRARY_HEX =
  /\b(?:text|bg|border|ring|from|to|via|shadow)-\[#[0-9a-fA-F]{3,8}\]/g;

const HEX_ALLOWED: Record<string, string[]> = {
  "app/[locale]/live/[steamId]/LiveScene.tsx": [
    "#1a1320",
    "#120c18",
    "#0c0810",
  ],
...
describe("design tokens", () => {
  ...
  it("components use token utilities, not raw Tailwind palette classes", () => {
    ...
    expect(
      hits,
      "Use a token utility (text-success, bg-danger/10, text-fg-muted...) instead. See CONTRIBUTING.md, Colors.",
    ).toEqual([]);
  });
  ...
  it("components do not hardcode hex colors in classes", () => {
    ...
    for (const f of files) {
      ...
      const src = readFileSync(f, "utf8");
      for (const m of [
        ...src.matchAll(ARBITRARY_HEX),
        ...src.matchAll(HEX_IN_CLASS),
      ]) {
    ...
    expect(
      hits,
      "Add a token in globals.css instead of a hex color. See CONTRIBUTING.md, Colors.",
    ).toEqual([]);
  });
});
