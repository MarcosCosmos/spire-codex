...
describe("redirectMissingEntity", () => {
  it("returns a real 404 for an unknown id instead of redirecting to the hub", () => {
    expect(() => redirectMissingEntity("cards", "no_such_card")).toThrow(
      "NEXT_NOT_FOUND",
    );
  ...
  it("308s a documented rename to the new id, keeping the locale prefix", () => {
    ...
    expect(() =>
      redirectMissingEntity("cards", "old_slug", "fra", legacy),
    ).toThrow("NEXT_REDIRECT:/fra/cards/new_slug");
    expect(() =>
      redirectMissingEntity("cards", "old_slug", undefined, legacy),
    ).toThrow("NEXT_REDIRECT:/cards/new_slug");
    expect(() =>
      redirectMissingEntity("relics", "old_slug", "fra", legacy),
