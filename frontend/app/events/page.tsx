...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (_lang) {
    ...
    description = `${gameName} ${t(CATEGORY_LABEL, lang)} (${nativeName}). Every shrine, Ancient, and story event, choices, dialogue, relic offerings, and outcomes.`;
  } else {
    let count = "66";
    try {
...
export default async function EventsPage({ params }: Props) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <RecentlyAdded entityType="events" label="Event" pathPrefix={`${prefix}/events`} />
