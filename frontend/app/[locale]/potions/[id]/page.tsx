...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/potions/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
      return buildPageMetadata({
        locale,
        path,
        title: t("Potion Not Found"),
        noIndex: true,
      });
  ...
  try {
    ...
    if (res.ok) {
      ...
      const detailJsonLd = buildDetailPageJsonLd({
        name: potion.name,
        description:
          desc || entityFallbackDescription(locale, potion.name, "potion"),
        ...
        breadcrumbs: [
          ...
          {
            name: uiText(locale, "Potions"),
            href: localePath(locale, "/potions"),
          },
      ...
      const faqQuestions = [
        {
          question: `What does ${potion.name} do in Slay the Spire 2?`,
          answer: desc || `${potion.name} is a potion in Slay the Spire 2.`,
        },
        {
          question: `How rare is ${potion.name}?`,
          answer: `${potion.name} is a ${potion.rarity} potion.`,
        },
      ];
      jsonLd =
        locale === "eng"
