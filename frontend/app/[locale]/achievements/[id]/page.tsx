...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/achievements/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
      return buildPageMetadata({
        locale,
        path,
        title: t("Achievement Not Found"),
        noIndex: true,
      });
  ...
  try {
    ...
    if (res.ok) {
      ...
      const detailJsonLd = buildDetailPageJsonLd({
        ...
        description:
          desc ||
          entityFallbackDescription(locale, achievement.name, "achievement"),
        ...
        breadcrumbs: [
          ...
          {
            name: achievement.name,
            href: localePath(locale, `/achievements/${id}`),
          },
        ],
      });
      const faqQuestions = [
