...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/afflictions/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
      return buildPageMetadata({
        locale,
        path,
        title: t("Affliction Not Found"),
        noIndex: true,
      });
  ...
  try {
    ...
    if (res.ok) {
      ...
      const detailJsonLd = buildDetailPageJsonLd({
        ...
        description:
          desc ||
          entityFallbackDescription(locale, affliction.name, "affliction"),
        ...
        breadcrumbs: [
          ...
          {
            name: affliction.name,
            href: localePath(locale, `/afflictions/${id}`),
          },
        ],
      });
      const faqQuestions = [
        {
          question: `What does ${affliction.name} do in Slay the Spire 2?`,
          answer:
            desc || `${affliction.name} is an affliction in Slay the Spire 2.`,
        },
        ...(affliction.is_stackable
          ? [
              {
                question: `Is ${affliction.name} stackable?`,
                answer: `Yes, ${affliction.name} is stackable.`,
              },
            ]
          : []),
      ];
