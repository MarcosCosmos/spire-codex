...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/characters/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
    ...
    const stats = char.starting_hp
      ? ` ${char.starting_hp} HP, ${char.max_energy} Energy.`
      : "";
    return buildPageMetadata({
      locale,
      path,
  ...
  try {
    ...
    if (res.ok) {
      ...
      const detailJsonLd = buildDetailPageJsonLd({
        name: char.name,
        description:
          desc || entityFallbackDescription(locale, char.name, "character"),
      ...
      const faqQuestions = [
        {
          question: `How do you play ${char.name} in Slay the Spire 2?`,
          answer:
            desc || `${char.name} is a playable character in Slay the Spire 2.`,
        },
        {
          question: `What is ${char.name}'s starting HP in Slay the Spire 2?`,
          answer: char.starting_hp
            ? `${char.name} starts with ${char.starting_hp} HP.`
            : `${char.name}'s HP information is available on the character page.`,
        },
        {
          question: `What type of deck does ${char.name} use?`,
          answer: char.deck?.length
            ? `${char.name} starts with ${char.deck.length} cards in their starting deck.`
            : `${char.name} uses a unique card pool.`,
        },
      ];
      jsonLd =
        locale === "eng"
