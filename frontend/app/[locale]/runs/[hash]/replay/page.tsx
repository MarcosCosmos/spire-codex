...
const fetchRun = cache(async (hash: string): Promise<ReplayRunInfo | null> => {
  ...
  if (!body || typeof body !== "object")
    throw new Error(`run API returned no object for ${hash}`);
...
const fetchCharacterNames = cache(
  async (locale: string): Promise<Record<string, string>> => {
    try {
      const res = await fetch(
        `${API_INTERNAL}/api/translations?lang=${locale}`,
        { next: { revalidate: 300 } },
      );
      if (!res.ok) return {};
...
function characterOf(
  ...
  const raw =
    run.players?.[run.player_index ?? 0]?.character ??
    run.players?.[0]?.character ??
    "";
  const bare = raw.replace("CHARACTER.", "");
  if (!bare) return "";
  return (
    names[bare.toLowerCase()] || bare.charAt(0) + bare.slice(1).toLowerCase()
  );
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const [run, characterNames] = await Promise.all([
    fetchRun(hash),
