...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/events/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
  ...
  try {
    ...
    if (res.ok) {
      ...
      const detailJsonLd = buildDetailPageJsonLd({
        name: event.name,
        description:
          desc || entityFallbackDescription(locale, event.name, "event"),
      ...
      const faqQuestions = [
        {
          question: `What happens in the ${event.name} event in Slay the Spire 2?`,
          answer: desc || `${event.name} is an event in Slay the Spire 2.`,
        },
        {
          question: `What type of event is ${event.name}?`,
          answer: `${event.name} is a ${event.type} event${event.act ? ` found in ${event.act}` : ""}.`,
        },
      ];
      if (event.options?.length) {
        faqQuestions.push({
          question: `What choices does ${event.name} offer?`,
          answer: `${event.name} offers ${event.options.length} choice(s): ${event.options.map((o: { title: string }) => o.title).join(", ")}.`,
        });
      }
      jsonLd =
        locale === "eng"
