...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/enchantments/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
      return buildPageMetadata({
        locale,
        path,
        title: t("Enchantment Not Found"),
        noIndex: true,
      });
    ...
    return buildPageMetadata({
      ...
      ogType: "article",
      image: enchantment.image_url
        ? imageUrl(enchantment.image_url)
        : undefined,
    });
  } catch {
    return buildPageMetadata({
...
export default async function Page({ params }: Props) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const detailJsonLd = buildDetailPageJsonLd({
        ...
        description:
          desc ||
          entityFallbackDescription(locale, enchantment.name, "enchantment"),
        ...
        breadcrumbs: [
          ...
          {
            name: uiText(locale, "Enchantments"),
            href: localePath(locale, "/enchantments"),
          },
          {
            name: enchantment.name,
            href: localePath(locale, `/enchantments/${id}`),
          },
        ],
      });
      const faqQuestions = [
        {
          question: `What does ${enchantment.name} do in Slay the Spire 2?`,
          answer:
            desc ||
            `${enchantment.name} is an enchantment in Slay the Spire 2.`,
        },
        {
          question: `What card type is ${enchantment.name} for?`,
          answer: enchantment.applicable_to
            ? `${enchantment.name} can be applied to ${enchantment.applicable_to}.`
            : enchantment.card_type
              ? `${enchantment.name} can be applied to ${enchantment.card_type} cards.`
              : `${enchantment.name} can be applied to any card type.`,
        },
      ];
      jsonLd =
        locale === "eng"
  ...
  if (!enchantment) redirectMissingEntity("enchantments", id, locale);
  return (
    <>
