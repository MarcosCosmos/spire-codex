...
export async function generateMetadata({
  ...
  const shortDescription = t(
    "Every {game} potion ranked S through F by community win-rate data.",
    { game: gameNameFor(locale, "Slay the Spire 2") },
  );
  return {
    title: `${t("Potion Tier List")} - ${t("All 63 Potions Ranked")} - ${game} | ${SITE_NAME}`,
    description: t(
      "Every {game} potion ranked S through F by community win rate. Codex Score with Bayesian shrinkage. Updated every 30 minutes.",
      { game },
    ),
    alternates: {
      canonical: `${SITE_URL}${localePath(locale, "/tier-list/potions")}`,
      languages: buildLanguageAlternates("/tier-list/potions"),
    },
    openGraph: {
      title: shortTitle,
...
async function fetchData(
  ...
  try {
    ...
    const [potionsRes, scoresRes] = await Promise.all([
      fetch(`${API_INTERNAL}/api/potions${langQuery(locale)}`, {
        next: { revalidate: 1800 },
      }),
      fetch(scoresUrl, { next: { revalidate: 300 } }),
    ]);
    const potions = potionsRes.ok
      ? ((await potionsRes.json()) as ApiPotion[])
      : [];
  ...
  const jsonLd = [
    buildBreadcrumbJsonLd([
      ...
      { name: t("Tier List"), href: localePath(locale, "/tier-list") },
      {
        ...
        href: localePath(locale, "/tier-list/potions"),
      },
    ]),
    buildCollectionPageJsonLd({
      name: t("Potion Tier List"),
      description: t(
        "Every {game} potion ranked by Codex Score from community-submitted run win rates.",
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-baseline gap-3 mb-2 flex-wrap">
        ...
        <span className="text-sm text-[var(--text-muted)]">
          {t("{n} potions", { n: entities.length.toLocaleString() })}
        </span>
      </div>
