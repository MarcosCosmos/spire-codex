...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/orbs/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
      return buildPageMetadata({
        locale,
        path,
        title: t("Orb Not Found"),
        noIndex: true,
      });
...
export default async function Page({ params }: Props) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const faqQuestions = [
        {
          question: `What does the ${orb.name} orb do in Slay the Spire 2?`,
          answer: desc || `${orb.name} is an orb in Slay the Spire 2.`,
        },
      ];
      jsonLd =
        locale === "eng"
