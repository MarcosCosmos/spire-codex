...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/powers/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
      return buildPageMetadata({
        locale,
        path,
        title: t("Power Not Found"),
        noIndex: true,
      });
  ...
  try {
    ...
    if (res.ok) {
      ...
      const detailJsonLd = buildDetailPageJsonLd({
        name: power.name,
        description:
          desc || entityFallbackDescription(locale, power.name, "power"),
        ...
        breadcrumbs: [
          ...
          {
            name: uiText(locale, "Powers"),
            href: localePath(locale, "/powers"),
          },
      ...
      const faqQuestions = [
        {
          question: `What does ${power.name} do in Slay the Spire 2?`,
          answer: desc || `${power.name} is a power in Slay the Spire 2.`,
        },
        {
          question: `Is ${power.name} a buff or debuff?`,
          answer: `${power.name} is a ${power.type} with ${power.stack_type} stacking.`,
        },
      ];
      jsonLd =
        locale === "eng"
