...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const res = await fetch(
      `${API_INTERNAL}/api/modifiers/${id}${langQuery(locale)}`,
    );
    if (!res.ok)
      return buildPageMetadata({
        locale,
        path,
        title: t("Modifier Not Found"),
        noIndex: true,
      });
  ...
  try {
    ...
    if (res.ok) {
      ...
      const detailJsonLd = buildDetailPageJsonLd({
        name: modifier.name,
        description:
          desc || entityFallbackDescription(locale, modifier.name, "modifier"),
      ...
      const faqQuestions = [
        {
          question: `What does the ${modifier.name} modifier do in Slay the Spire 2?`,
          answer:
            desc || `${modifier.name} is a run modifier in Slay the Spire 2.`,
        },
      ];
      jsonLd =
        locale === "eng"
